...
## 3. Store the OpenAI key as an environment variable

```bash
netlify secrets:set OPENAI_API_KEY <your-openai-key>
```

You can also add the variable in the Netlify UI: **Site settings → Build & deploy → Environment**.

The function at `netlify/functions/openai.js` reads this value from `process.env.OPENAI_API_KEY`, so the key never ships to browsers.
